...
/*1) Inizializza la lista pcbFree in modo da contenere 
tutti gli elementi della pcbFree_table.
Questo metodo  deve essere chiamato una volta sola 
in fase di inizializzazione della struttura dati.*/
void initPcbs();

/*2) Inserisce il PCB puntato da p nella lista
dei PCB liberi (ovvero pcbFree_h)*/
void freePcb(pcb_t * p);

/*3) Restituisce NULL se la pcbFree_h è vuota.
...
/*5) Restituisce TRUE (1) se la lista puntata da head è vuota, FALSE (0) altrimenti.*/
int emptyProcQ(struct list_head *head);

/*6) Inserisce l’elemento puntato da p nella coda dei processi puntata da head.*/
void insertProcQ(struct list_head* head, pcb_t* p);

/*7) Restituisce l’elemento di testa della coda dei processi 
da head, SENZA RIMUOVERLO. Ritorna NULL se la coda non ha elementi.*/
pcb_t* headProcQ(struct list_head* head);

/*8) Rimuove il primo elemento dalla coda dei processi puntata da head. 
Ritorna NULL se la coda è vuota. Altrimenti ritorna 
...
/*10) Restituisce TRUE se il PCB puntato da p
non ha figli, FALSE altrimenti.*/
int emptyChild(pcb_t *p);

/*11) Inserisce il PCB puntato da p come figlio
del PCB puntato da prnt.*/
void insertChild(pcb_t *prnt,pcb_t *p);

/*12) Rimuove il primo figlio del PCB puntato da p. 
